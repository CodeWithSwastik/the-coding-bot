...
def is_staff_or_helper():
    return commands.has_any_role("Staff", "Official Helper")  # Staff + Helper

class Help(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.help_rules_channel = 754712400757784709
        
    @property
    def read_the_rules_role(self):
        return self.bot.tca.get_role(903133599715459153)

    @property
    def help_ban_role(self):
        return self.bot.tca.get_role(903133405317857300)

    @commands.Cog.listener()
    async def on_interaction(self, interaction):
        if interaction.channel_id != self.help_rules_channel:
            return

        if interaction.data.get("custom_id", "") == "help_button":
            if self.help_ban_role in interaction.user.roles:
                return await interaction.response.send_message(
                    "You are banned from accessing the help channels. Please open a ticket to appeal your ban.",
                    ephemeral=True
                )
            await interaction.user.add_roles(self.read_the_rules_role)
            await interaction.response.defer()

    @commands.command(name="help-ban")
    @is_staff_or_helper()
    async def help_ban(self, ctx, user: discord.Member):
        if ctx.author.top_role <= user.top_role:
            return await ctx.send(f'You cannot do this as that person has a higher role than you.')    

        await user.remove_roles(self.read_the_rules_role)
        await user.add_roles(self.help_ban_role)
        await ctx.send(f'{user.mention} has been banned from the help channels.')    

    @commands.command(name="help-unban")
    @is_staff_or_helper()
    async def help_unban(self, ctx, user: discord.Member):
        if ctx.author.top_role <= user.top_role:
            return await ctx.send(f'You cannot do this as that person has a higher role than you.')    

        await user.add_roles(self.read_the_rules_role)
        await user.remove_roles(self.help_ban_role)
        await ctx.send(f'{user.mention} has been unbanned from the help channels.')    
...
# >jsk py
# ```
# embed = discord.Embed(description = 'Press the button below to agree to the rules.', color = discord.Color.yellow())
# button = discord.ui.Button(label='I agree!', emoji='🙋', custom_id='help_button', style=discord.ButtonStyle.green)
